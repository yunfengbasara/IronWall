...
export interface SkillDef {
  ...
  reach: number;
  /**
   * 这一招每升一级，作用距离多涨多少。不写就用全局那一档（SKILL_LEVEL_REACH = 0.04）。
   *
   * 全局那一档是**故意平的**：一招的形状是它的身份，练到后来变成另一个东西是坏的。所以这
   * 一项只该在"这一招的强弱几乎全压在面积上"的时候才给 —— 目前只有破空，见那条记录。
   */
  reachGrowth?: number;
...
export const Skills: SkillDef[] = [
  ...
  },
  {
     ...
     *
     * **这个数被砍过三刀，而三刀比的都是错的东西。** 4.5 → 3.3 → 2.9 → 2.4，每一次的理由
     * 写的都是"够得太远、一发捧到半屏外"—— 比的全是**距离**，一次都没比过**面积**。而破空的
     * 张角只有 52°（全场最窄），半径缩一成，面积就缩两成。按面积摆出来是这样的：
     *
     *   reach 4.5   面积 2333  = 武将横扫的 113%
     *   reach 3.3   面积 1255  = 61%
     *   reach 2.9   面积  969  = 47%
     *   reach 2.4   面积  664  = 32%   ← 三刀砍完
     *
     * 割草游戏里一招的强弱是**每秒抹掉多少人**，而那跟着面积走，不跟着半径走。离线把四个
     * 角色横着量，剑士的清场速度是武将的 40%（1.5 : 3.8 个/秒），全场垫底一大截 —— 那不是
     * 打法差异，是单纯更弱。
     *
     * 他因此**重新拿回"够得最远"这块牌子**，而这是对的：他手里是最短的剑，但破空本来就不是
     * 剑砍出去的，是一道甩出去的波 —— 用兵器长度去限制一道波，才是当初那三刀真正的错处。
     *
     * 身份仍然是**窄而且会跑**：一条 52° 的走廊横穿人堆，而横扫是身前 80° 的一大片。同样的
     * 面积，一个是长而窄、要对准，一个是短而宽、糊在脸前。
     *
     * ---
     *
     * **但一口气给到 3.1 是给早了。** 实打下来后期没问题，前期太狂 —— 因为全局那条作用距离
     * 的成长只有 0.04（满级 ×1.16），等于这一招从 1 级起就基本是满尺寸的，而剑士的强度几乎
     * 全压在它的面积上。于是他开局就带着一个和武将满级扇面一样大的东西，再叠上全场最高的
     * 暴击和最快的攻击力成长。
     *
     * 所以底数压回 2.45，另给它一条**自己的、陡一倍的**成长（reachGrowth 0.12，全局是 0.04）。
     * 两头都落在该落的地方：
     *
     *   1 级   半径 54   面积 1307 = 武将同级的 63%   ← 开局明显小一圈，得贴近了打
     *   3 级   半径 67   面积 2008 = 83%
     *   5 级   半径 80   面积 2864 = 103%            ← 练满了才追平
     *
     * 0.12 这个斜率是**从两头倒解出来的**：一头钉在"1 级要明显弱于武将"，另一头钉在"满级
     * 回到上一版那个已经验证过好用的尺寸"（上一版是 3.1 拉平，满级面积 2931）。先试过 0.08，
     * 满级只有 2372 —— 那是把后期也一起砍了，而后期本来没问题。
     *
     * 这一条同时把这个角色的曲线**说清楚了**：他是四个人里唯一一个"开局最弱、练满最强"的，
     * 而那正好就是进攻型该有的形状 —— 前期薄得像纸（900 血、全场最低防），得先熬过去。
     */
    reach: 2.45,
    reachGrowth: 0.12,
    arc: 0.9,
